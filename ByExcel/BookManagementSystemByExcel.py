...
# 读excel文件
def read_excel(books):
    bookList.clear()
    if not os.path.exists('书籍信息.xls'):
        return
    workbook = xlrd.open_workbook('书籍信息.xls')  # 打开表格文件
    worksheet = workbook.sheets()[0]  # 获取第1张张工作表
    row_number = worksheet.nrows  # 工作表行数
    for i in range(1, row_number):
        id = worksheet.cell(i, 0).value  # 获取序号
        name = worksheet.cell(i, 1).value  # 获取书名
        author = worksheet.cell(i, 2).value  # 获取作者
        press = worksheet.cell(i, 3).value  # 获取出版社
        type = worksheet.cell(i, 4).value  # 获取类别
        book = Book(id, name, author, press, type)  # 创建一本新的书籍
        books.append(book)  # 将新创建的书籍放入书籍列表中


# 写excel文件
def write_excel(books):
    workbook = xlwt.Workbook(encoding='utf-8')  # 设置编码格式
    worksheet = workbook.add_sheet('书籍信息')  # 设置工作表名称
    # 将标题信息写入工作表中
    worksheet.write(0, 0, 'id')
    worksheet.write(0, 1, 'name')
    worksheet.write(0, 2, 'author')
    worksheet.write(0, 3, 'press')
    worksheet.write(0, 4, 'type')
    cnt = 0  # 记录当前书籍的个数
    # 遍历书籍列表，将其信息写入工作表中
    for book in books:
        cnt += 1
        worksheet.write(cnt, 0, book.get_id())
        worksheet.write(cnt, 1, book.get_name())
        worksheet.write(cnt, 2, book.get_author())
        worksheet.write(cnt, 3, book.get_press())
        worksheet.write(cnt, 4, book.get_type())
    workbook.save('书籍信息.xls')  # 保存文件
